...
- **[`@path` imports and ONECODE.md in the claudeMd block](decisions/tools.md#path-imports-and-onecodemd-in-the-claudemd-block-2026-08-20)** — CLAUDE.md `@path` imports expanded in place (recursive, depth-capped, cycle/code-span safe) so `@AGENTS.md` reuse works; `ONECODE.md`/`onecode.md`/`OneCode.md` (global `~/.one-code` + per-dir) carry One Code-only instructions Claude Code never reads, riding their **own `# oneCodeMd` block above `# claudeMd`** so they take precedence over CLAUDE.md (keeping `# claudeMd` byte-exact with CC); AGENTS.md is a **per-directory fallback for a missing CLAUDE.md** (CLAUDE.md > AGENTS.md, first-match like Codex/opencode but CLAUDE-preferred), surfaced in the startup banner; all inert (byte-identical) when unused.
...
- **[Bundled Claude Code built-in skills](decisions/skills-plugins.md#bundled-claude-code-built-in-skills-2026-08-20)** — CC's built-ins ship inside its binary (not on disk), so One Code bundles `simplify`/`code-review`/`security-review`/`fewer-permission-prompts` under `skills/` (discovered last so user skills win); bodies extracted from the CC binary and adapted (Agent tool, JSON-array output, `~/.one-code` writes); extractor + procedure in `.claude/skills/extract-cc-skills/`; Artifact/harness/account skills deliberately excluded.
...
- **[Interrupted-turn line, and suppressing pi's "Operation aborted"](decisions/tui.md#interrupted-turn-line-and-suppressing-pis-operation-aborted-2026-08-20-unreviewed)** *(unreviewed)* — CC's dim `Interrupted · What should One Code do instead?` on Esc (display-only appendEntry keyed off the last assistant message's `stopReason:"aborted"` via `lib/interrupt.ts`); turn-duration suppresses `✻ Cooked for …` on the same signal. pi's own red "Operation aborted" is unreachable from the extension API (pi sets `errorMessage` after extension handlers run), so `app/bin.mjs` monkeypatches `AssistantMessageComponent.updateContent` to skip it — app-only, like the exit patch.
