...
- **Compaction uses Claude Code's own prompt** for `/compact` and automatic compaction alike, keeping only the `<summary>` block. Like Claude Code, it runs on the session model with the session's system prompt, tools, and message prefix intact, so the provider prompt cache pays for most of the call. `/compact <instructions>` and CLAUDE.md compact-instruction sections are honored; any failure falls back to pi's built-in compaction, and `CC_COMPACTION=0` opts out.
