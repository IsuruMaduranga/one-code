...
import { GIT_STATUS_META_ARGS, gitStatusMeta, wantsGitStatusMeta } from "../auto-mode/git-status-meta.ts";
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		if (mode === "auto") {
			...
			if (shell && recordedCommand && wantsGitStatusMeta(shell, recordedCommand)) {
				const porcelain = await gitStatusOutput(callCwd, GIT_STATUS_META_ARGS);
				if (porcelain !== undefined) transcript.push({ kind: "meta", gitStatus: gitStatusMeta(porcelain) });
			}
