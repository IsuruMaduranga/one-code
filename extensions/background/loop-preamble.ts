/**
 * Claude Code's autonomous-loop instructions (2.1.282, verbatim): the text a
 * bare `/loop` (or `/loop <interval>` with no prompt) runs on, when no
 * loop.md tasks file exists. There are two; the
 * persistent one is chosen by `CLAUDE_CODE_LOOP_PERSISTENT` (Claude Code also
 * reads a remote flag, default off). Findings §21.
 *
 * Verbatim; never hand-edit the text.
 */
...
export const AUTONOMOUS_PREAMBLE_PERSISTENT = `# Autonomous loop check
