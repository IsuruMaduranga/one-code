/**
 * The `powershell` tool's model-facing description: Claude Code's own text
 * (pure).
 *
 * The body is the 2.1.276 capture, byte for byte
 * (`tools/powershell-tool-2.1.276-macos-pwsh7.json`, locked by
 * `test/unit/powershell-description.test.ts`); the edition section is the
 * one slot Claude Code fills per machine — PowerShell 7+ text captured, the
 * Windows PowerShell 5.1 and unknown-edition texts from the binary's JS
...
/** Claude Code's edition sections (binary JS, 2.1.276). */
export const EDITION_SECTIONS: Record<PowerShellEditionName, string> = {
	core: `PowerShell edition: PowerShell 7+ (pwsh)
   - Pipeline chain operators \`&&\` and \`||\` ARE available and work like bash. Prefer \`cmd1 && cmd2\` over \`cmd1; cmd2\` when cmd2 should only run if cmd1 succeeds.
   - Ternary (\`$cond ? $a : $b\`), null-coalescing (\`??\`), and null-conditional (\`?.\`) operators are available.
   - Default file encoding is UTF-8 without BOM.`,
	desktop: `PowerShell edition: Windows PowerShell 5.1 (powershell.exe)
   ...
   - \`ConvertFrom-Json\` returns a PSCustomObject, not a hashtable. \`-AsHashtable\` is not available.`,
	unknown: `PowerShell edition: unknown — assume Windows PowerShell 5.1 for compatibility
   - Do NOT use \`&&\`, \`||\`, ternary \`?:\`, null-coalescing \`??\`, or null-conditional \`?.\`. These are PowerShell 7+ only and parser-error on 5.1.
   - To chain commands conditionally: \`A; if ($?) { B }\`. Unconditionally: \`A; B\`.`,
};

/** The captured description with the edition section replaced by a slot. */
const TEMPLATE = `Executes a given PowerShell command with optional timeout. Working directory persists between commands; shell state (variables, functions) does not.
